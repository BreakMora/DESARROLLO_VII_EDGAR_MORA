...
function impartir_Tutoria($estudiantes){
    $tutorias = [];
    foreach ($estudiantes as &$estudiante) {
        $promedio = calcularPromedio($estudiante["calificaciones"]);
        if ($promedio <= 75) {
            $tutorias[] = $estudiante["nombre"];
        }
    }
    return implode(", ", $tutorias);;
}

echo "<h3> Lista de estudiantes que requieren de tutorias.</h3>";
echo impartir_Tutoria($estudiantes);
...
function estudiantes_Honor($estudiantes){
    ...
    return implode(", ", $honor);
}

echo "<h3> Lista de estudiantes de Honor.</h3>";
echo estudiantes_Honor($estudiantes);
