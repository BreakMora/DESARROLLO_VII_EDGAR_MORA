...
$ventas = [
    "Norte" => [
        "Producto A" => [100, 120, 140, 110, 130],  //(100-120)/100 *100   =  -20%
...
function crecimientoVentas($ventas){
    ...
    foreach ($ventas as $region => $productos){
        ...
        foreach ($productos as $producto => $ventas){
            ...
            for ($i=0; $i<count($ventas)-1; $i++){
                $porcentaje = (($ventas[$i + 1] - $ventas[$i]) / $ventas[$i]) * 100;
                echo "<br>Mes " . ($i + 1) . " a Mes " . ($i + 2) . ": " . number_format($porcentaje, 2) . "%";
                
                if($porcentaje > $maxCrecimiento ){
                    $maxCrecimiento = $porcentaje;
    ...
    echo "La region " . $maxRegion . " y el producto " . $maxProducto . " tuvieron el mayor crecimiento con un " . number_format($maxCrecimiento, 2) . "%";
}
