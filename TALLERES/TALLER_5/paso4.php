...
function libros_Autor($biblioteca, $autor){
    return array_filter($biblioteca, function($libro) use ($autor){
...
function buscarLibros($biblioteca, $termino) {
    return array_filter($biblioteca, function($libro) use ($termino){
        return  stripos($libro["titulo"], $termino) == true ||
                stripos($libro["autor"], $termino) == true ||
                stripos($libro["genero"], $termino) == true;
    });
}
echo "<br><br>";
...
function generarReporteBiblioteca($biblioteca) {
    $cantidadLibros = count($biblioteca);
    $prestados = 0;
    $libroGenero = [];
    $autores = [];

    foreach ($biblioteca as $libro) {
        if (isset($libro["prestado"]) && $libro["prestado"] === true) {
            $prestados++;
        }

        $genero = $libro["genero"];
        if (isset($libroGenero[$genero])) {
            $libroGenero[$genero]++;
        } else {
            $libroGenero[$genero] = 1;
        }
    ...
    $autorConMasLibros = array_keys($autores, max($autores))[0];

    $reporte = "Total de libros: $cantidadLibros <br>";
    $reporte .= "Libros prestados: $prestados <br>";
    $reporte .= "Libros por género:<br>";

    foreach ($libroGenero as $genero => $cantidad) {
        $reporte .= " - $genero: $cantidad<br>";
    }

    $reporte .= "Autor con más libros: $autorConMasLibros<br>";
